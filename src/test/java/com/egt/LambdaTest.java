...
//	}
	...
	void 람다_메소드레퍼런스() {
		...
		personList.add(new Person("김진혁", 27));
		personList.add(new Person("김바다", 7));
		
		// 객체 안에 있는 메소드 호출
		personList.forEach(p -> p.printInfo()); // 람다
		personList.forEach(Person::printInfo); // 메소드 래퍼런스
		
		//객체를 println으로 출력
		personList.forEach(p -> System.out.println(p)); // 람다
		personList.forEach(System.out::println); // 메소드 래퍼런스
	}
